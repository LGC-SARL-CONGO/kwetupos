...
export default function Radio({ options: items, disabled, direction, selected, onChange }) {
    ...
    return <div className={styles['radio-container']} style={{
        flexDirection: direction || 'column',
        ...(direction === 'row' && {gap: '16px'})
    }}>
        {options?.map((option, index) => 
            <div key={index} className={`d-flex align-items-center ${option?.disabled ? styles['radio-container--disabled'] : ''}`}>
                <input 
                    type="radio" 
                    ref={inputRef}
