...
export default function DatePicker({value, label, placeholder, minDate, maxDate, valid,required, message, onChange}) {
        ...
        if (event) {
            ...
            setTimeout(() => {
                if (spaceAbove < spaceBelow) {
                    contentRef.current.classList.remove('date-picker-container__selector--dropup')
                } else {
                    contentRef.current.classList.add('date-picker-container__selector--dropup')
                }
            }, 1);
        }
    };
    ...
    const resetBorderColor = () => {
        inputRef.current.style.borderColor = theme === 'dark' ? 'rgba(255, 255, 255, .2)' : '#D0D5DD'
    }
    ...
    return <div className='date-picker-container' ref={containerRef}>
        <div className='date-picker-container__content'>
            <label htmlFor={inputRef} className='date-picker-container__content--label'>
                <span>{label}</span>
                
                {required && label && <span className='date-picker-container__content--label--required'>*</span>}
            </label>
