...
export default function DateTimerPicker({value, label, placeholder, minDate, maxDate, valid,required, message, onChange, placement}) {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (i === currentDate.getHours()) {
                hoursList.push({ hour: formattedHour, current: hour ? false : true, ...(hour && Number(hour) === i && {selected: true}) })
            } else { 
                hoursList.push({ hour: formattedHour, current: false, ...(hour && Number(hour) === i && {selected: true}) })
            }
        }
        ...
        for (let i = 0; i < 60; i++) {
            ...
            if (i === currentDate.getMinutes()) {
                minutesList.push({ minute: formattedMinute, current: minute ? false : true, ...(minute && Number(minute) === i && {selected: true}) })
            } else {
                minutesList.push({ minute: formattedMinute, current: false, ...(minute && Number(minute) === i && {selected: true}) })
            }
        }
        ...
        if (event) {
            ...
            setTimeout(() => {
                if (spaceAbove < spaceBelow) {
                    contentRef.current.classList.remove('date-timer-picker-container__selector--dropup')
                } else {
                    contentRef.current.classList.add('date-timer-picker-container__selector--dropup')
                }
            }, 1);
        }
    };
    ...
    const onSelected = value => {
        const formattedHour = currentDate.getHours() < 10 ? '0' + currentDate.getHours() : currentDate.getHours()
        const formattedMinute = currentDate.getMinutes() < 10 ? '0' + currentDate.getMinutes() : currentDate.getMinutes()
        ...
        if (!state?.inputDate?.includes(':')) {
            ...
            minutesList.forEach(item => {
                if (Number(item.minute) === currentDate.getMinutes()) item.selected = true
            })
    ...
    const scrollSelectedDateTimer = () => {
        ...
        if (hourElement) {
            hourElement.scrollTo({ top: 40 * (hour ? hour : currentDate.getHours()), behavior: 'instant' });
        }

        if (timerElement) {
            timerElement.scrollTo({ top: 40 * (minute ? minute : currentDate.getMinutes()), behavior: 'instant' });
        }
    }
    ...
    return <div className='date-timer-picker-container' ref={containerRef}>
        <div className='date-timer-picker-container__content'>
            <label htmlFor={inputRef} className='date-timer-picker-container__content--label'>
                <span>{label}</span>
                
                {required && label && <span className='date-timer-picker-container__content--label--required'>*</span>}
            </label>
            ...
            {(state?.valid !== undefined && message) && <span className={`content-message ${statusClass()}`}>{message || 'Required field'}</span>}
        </div>
        ...
        >
           <div className="d-flex">
                ...
                <div className='date-timer-picker-container__selector--timer' ref={timerRef}>
                    <div className='date-timer-picker-container__selector--timer--hour' style={{
                    ...
                    >
                        {hours.map((hour, index) => (
                            <span key={index} className={`${hour.current ? 'date-timer-picker-container__selector--timer--current' : ''} ${hour.selected ? 'date-timer-picker-container__selector--timer--selected' : ''}`} onClick={() => selectTime(hour.hour, index)}>{hour.hour}</span>
                        ))}
                    </div>

                    <div className='date-timer-picker-container__selector--timer--time' style={{
                            height: `${timerHeight}px`,
                            maxHeight: `${timerHeight}px`
                        }}
                        onScroll={infinityScroll}
                    >
                        {minutes.map((minute, index) => (
                            <span key={index} className={`${minute.current ? 'date-timer-picker-container__selector--timer--current' : ''} ${minute.selected ? 'date-timer-picker-container__selector--timer--selected' : ''}`} onClick={() => selectMinute(minute.minute, index)}>{minute.minute}</span>
                        ))}
                    </div>
                </div>
           </div>
