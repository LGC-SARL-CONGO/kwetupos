...
import { Drawer, DrawerHeader, DrawerBody, DrawerFooter } from '../../BaseUI/Drawer';
...
export default function AppPage({title, breadcrumbs, actions, children, className, onClick}) {
    const theme = useTheme()
    const { setTheme, sidebarTheme, setSidebarTheme, topMenuTheme, setTopMenuTheme } = useContext(ThemeContext);

    const [isOpen, setIsOpen] = useState(false)

    return <div className={`app-page-container ${className}`} onClick={onClick} data-theme={theme}>
        <div className='d-flex align-items-end justify-content-between py-4'>
            <div className='d-flex flex-column gap-1'>
                {Object.keys(breadcrumbs || [])?.length > 0 && <div>
                    <Breadcrumbs options={breadcrumbs} />
                </div>}
                <h1 className={`${theme === 'dark' ? 'page-label-dark' : 'page-label'}`}>{title}</h1>
            </div>
