...
export default function VideoPlayer({file}) {
    ...
    return <>
        {url && <div 
            ...
            <div className={styles['video-player-container__controls']} style={{ opacity: showControls ? 1 : 0 }}>
                <div className={styles['video-player-container__controls__control']}>
                    <div className={`${styles['video-player-container__controls__control--play']}`} onClick={() => toogleVideoPlay()}>
                        {playerState.playing ? < MdPause /> : <MdPlayArrow />}
                    </div>

                    <input 
                        type="range" 
                        className={styles['video-player-container__controls__control--progress']}
                        ref={progressBar}
                        min="0" 
                        max="100" 
                    ...
                    <div className={styles['video-player-container__controls__control--current-time']}>
                        <span>{playerState.currentTime}</span>
                    </div>

                    <div className={styles['video-player-container__controls__control--fullscreen']} onClick={() => toggleFullScreen()}>
                        {playerState.fullscreen ? <MdOutlineFullscreenExit /> : <MdOutlineFullscreen />}
                    </div>
                </div>
            </div>
        </div>}
