...
const Calendar = () => {
  ...
  useEffect(() => {
    ...
    storageEvents?.forEach((event) => {
      if (event.repeat !== "never") {
        event.rrule = {
          freq: event.repeat,
          dtstart: `${event?.start?.split("T")[0]}T${
  ...
  useEffect(() => {
    ...
    dayGridFiltered?.forEach((element) => {
      ...
      chidrens?.forEach((children) =>
        children.classList.add("my-custom-class")
      );
    });

    const moreLinkElements = Array.from(
      document.querySelectorAll(".fc-more-link")
    );
  }, [events]);
  ...
  const adjustDraggableEventDates = (event, field) => {
    ...
    if (field === "end") {
      ...
      return event?.end
        ...
        : `${start?.split("T")?.[0]}T${
            event.extendedProps?.endDate?.split("T")?.[1]
          }`;
    }
  };
    ...
    const dropdown = `<div class="dropdown-container-fc dropdown-container-fc__filter" id="dropdown-filter-fc">
            <ul class="dropdown-container-fc__filter__list">
                ${typesEvent
                  .map(
                    (event) =>
                      `<li class="list__item" onclick="getFilterFc(this, '${
                        event.type
                      }')">
                        <input type="checkbox" class="checkbox-fc ${
                          event.type
                        }" value="${event.type}" ${
                        eventTypeFilter.includes(event.type) ? "checked" : ""
                      } value="${event.type}"/>
  ...
  const injectDropdownChangeView = (event) => {
    ...
    const dropdown = `<div class="dropdown-container-fc dropdown-container-fc__change-view" id="dropdown-change-view-fc">
            ${options
              .map(
                (option) =>
                  `<div class="dropdown-container-fc__change-view--item ${
                    defaultView?.toLowerCase() === option.value
                      ? "dropdown-container-fc__change-view--item--active"
                      : ""
                  }" onclick="createChageViewFc(this, '${option.value}')">
                  ${option.label}
                </div>`
              )
              .join("")}
      ...
      if (startPeriod === endPeriod) {
        return (
          <div className="content__date content__info">
            ...
            {textTransform("lowercase", translate("CALENDAR.UNTIL"))} {endTime}
          </div>
        );
      } else {
        return (
        ...
        <div className="d-flex align-items-center justify-content-end gap-1 u-pointer mb-3 pr-3">
          {eventInfo.url && (
            <ActionButton
              ...
              title={translate("CALENDAR.MEETING_LINK")}
            >
          ...
          {eventInfo?.guests?.length > 0 && (
            <div className="content__guests content__info">
              ...
              <Accordion
                items={[
                  {
                    title: `${eventInfo?.guests?.length} ${translate(
                      "UI.GUESTS"
                    )}`,
                  },
                ]}
                ...
                <div className="content__guests content__info">
                  ...
                  >
                    <AccordionItem className="p-5">
                      {users
                        .filter((user) =>
                          eventInfo?.guests?.includes(user.value)
                        )
                        ?.map((user, index) => (
                          <div
                            key={index}
