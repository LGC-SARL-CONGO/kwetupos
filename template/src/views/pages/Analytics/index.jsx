...
const Analytics = () => {
    ...
    const perDevice = {
        series: [{
          ...
          options: {
            xaxis: {
              categories: [useTranslate('COUNTRIES.CHINA'), useTranslate('COUNTRIES.FRANCE'), useTranslate('COUNTRIES.ARGENTINA'), useTranslate('COUNTRIES.BRAZIL'), useTranslate('COUNTRIES.UNITED_STATE')]
            }
        },
    }
    

    const calculatePercentage = value => ((value / salesByRegion?.map(value => value?.quantity).reduce((a, b) => a + b, 0)) * 100)?.toFixed(2)
    
    return <AppPage title="Analytics" breadcrumbs={breadcrumbs}>
        <Container>
        ...
        <Container className='mt-3'>
            <ContainerItem sm={4} md={3} xl={4}>
                <Container>
                    ...
                    <ContainerItem sm={4} md={8} xl={12} className={styles['marketing__goal']}>
                        <AppChart 
                        ...
                        <div className={styles['marketing__goal__footer']}>
                            <div className={styles['info']}>
                                <span className={styles['info__label']}>{useTranslate('DASHBOARD.ORGANIC')}:</span>
                                <span className={styles['info__value']}>{formatValue(12389.90, 'money')}</span>
                            </div>

                            <div className={styles['info']}>
                                <span className={styles['info__label']}>{useTranslate('DASHBOARD.PAY')}:</span>
                                <span className={styles['info__value']}>{formatValue(16152.12, 'money')}</span>
                            </div>
                        </div>
                    </ContainerItem>
                </Container>
