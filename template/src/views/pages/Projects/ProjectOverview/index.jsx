...
import { extensionGroup, extensions } from "../../../../extensions";
...
export default function ProjectOverview() {
    const [comments, setComments] = useState([
        {
            ...
            image: '', 
            comment: 'Estou enfrentando alguns impeditivos nesta demanda. Alguém poderia me ajudar?',
            time: '2024-04-02'
        },
        {
    ...
    const files = [
        ...
        { name: 'Git Cheatsheet', type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' },
    ...
    const breadcrumbs = [
        {label: 'Magnun', url: ''},
        {label: 'Projects', url: ''},
        {label: translate('PROJECTS.PROJECT_OVERVIEW')},
    ]
    ...
    return <AppPage title={translate('PROJECTS.PROJECT_OVERVIEW')} breadcrumbs={breadcrumbs}>
        <Container>
            ...
            <ContainerItem sm={4} md={8} xl={4}>
                <CardContainer title={translate('PROJECTS.PROJECT_TEAM')} className='px-3'>
                    <ListGroup borderless={true}>
                        {contribuitors?.map((contribuitor, index) => (
                            <ListItem key={index}>
                                ...
                                <div>
                                    <Lozenge appearance='info-subtle' isBold isUpper>{contribuitor.team}</Lozenge>
                                </div>
                            </ListItem>
                        ))}
                    </ListGroup>
                ...
                <CardContainer title={translate('UI.COMMENTS')} className='px-3 pb-3'>
                   ...
                   {comments?.map((comment, index) => (
                        <Comment
                            ...
                            time={comment?.time}
                            actions={[
                                <CommentAction>{translate('EMAIL.TO_RESPOND')}</CommentAction>,
                            ...
                            <div key={index} className='file__custom'>
                                <div className='u-ellipsis'>
                                    <div className='d-flex align-items-center gap-1'>
                                        ...
                                        <div className='u-ellipsis' title={`${file.name}.${extensions[file?.type]}`}><span style={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>{file.name}</span>.{extensions[file?.type]}</div>
                                    </div>
                                </div>

                                <div>
                                    <span className="material-symbols-outlined u-pointer">download</span>
                                </div>
                            </div>
                        ))}
                    </div>
