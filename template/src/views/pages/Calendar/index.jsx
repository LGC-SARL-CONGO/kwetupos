...
import {Dropdown, DropdownContent, DropdownItem} from '../../../components/BaseUI/Dropdown';
...
import createdByImage from '../../../assets/users/beatriz.jpg'
...
const Calendar = () => {
    ...
    const [users, setUsers] = useState(usersJson.map(user => ({
        label: user.name,
        value: user.id
    })))
    ...
    const [recurringEventTypeExcluded, setRecurringEventTypeExcluded] = useState(1)

    const typesEvent = [
        { name: translate('CALENDAR.EVENT'), type: 'event', value: 'event-primary',},
        { name: translate('CALENDAR.REMINDER'), type: 'reminder', value: 'event-warning'},
        { name: translate('CALENDAR.TASK'), type: 'task', value: 'event-success'},
        { name: translate('CALENDAR.BIRTHDAY'), type: 'birthday', value: 'event-danger'},
    ]
    ...
    const daysOfWeek = [translate('DAYSOFWEEK.SUNDAY'), translate('DAYSOFWEEK.MONDAY'), translate('DAYSOFWEEK.TUESDAY'), translate('DAYSOFWEEK.WEDNESDAY'), translate('DAYSOFWEEK.THURSDAY'), translate('DAYSOFWEEK.FRIDAY'), translate('DAYSOFWEEK.SATURDAY')]
    ...
    useEffect(() => {
        ...
        const filteredEventTypes = JSON.parse(localStorage.getItem('filteredEventTypes'))
        ...
        if (storageEvents) {
            ...
            if (filteredEventTypes) {
                ...
                setFilteredEvents([...storageEvents?.filter(event => filteredEventTypes.includes(event.type))])
                return
            }
    ...
    useEffect(() => {
        ...
        const dayGridFiltered = Array.from(dayGridElements).filter(element => {
            const filhosComClasse = element.querySelectorAll('.fc-daygrid-event-harness')
        ...
        const moreLinkElements = Array.from(document.querySelectorAll('.fc-more-link'));
    }, [events])
    ...
    const formatDisplayDate = date => {
        ...
        if (date?.includes('00:00')) return dateParts[0].split('-').reverse().join('/')
    ...
    const handleEventClick = ({event, el, jsEvent}) => {
        ...
        setEventInfo({
            ...
            formattedStartDate: formatDisplayDate(formatDateToISOString(event?.start)),
            end: adjustDraggableEventDates(event, 'end'),
            endDate: adjustDraggableEventDates(event, 'end'),
            formattedEndDate: formatDisplayDate(formatDateToISOString(event?.end || event?.start)),
            id: event.id,
            url: event.url,
        ...
        setTimeout(() => {
            ...
            const {left, width, top, right} = element.getBoundingClientRect()
            const {width: dropdownWidth, height: dropdownHeight} = eventDropdowRef.current.getBoundingClientRect()
            const {width: calendarWidth} = fullCalendarRef.current.elRef.current.getBoundingClientRect()

            let elements = Array.from(document.body.querySelectorAll('[class]'))

            const hasAMobileMenu = elements.some(element => {
                let classes = element.className.baseVal !== undefined ? element.className.baseVal.split(' ') : element.className.split(' ')
            ...
            if (parseInt(left + (hasAMobileMenu ? (dropdownWidth + 70) / 2 : dropdownWidth / 2)) > parseInt(calendarWidth) ) {
            ...
            if (defaultView === 'Day') dropdownLeft = (window.innerWidth / 2) - (dropdownWidth / 2)
    ...
    const deleteEvent = event => {
        ...
        if (event.repeat !== 'never') {
            ...
            if (recurringEventTypeExcluded === 1) excludeDates = [...event.excludeDates, event?.start?.split('T')[0]]

            if (recurringEventTypeExcluded === 2) removeAllAfterCurrent = event?.start?.split('T')[0]
            ...
            setEvents([...values])
            persistEventsInLocalStorage({}, recurringEventTypeExcluded === 3 ? index : -1)
    ...
    const handleEventDraggable = ({ event }) => {
        const draggableIndex = events.findIndex(item => Number(item.id) === Number(event.id))
    ...
    const adjustDraggableEventDates = (event, field) => {
        ...
        if (field === 'end') {
            ...
            return event?.end ? formatDateToISOString(event?.end) : `${start?.split('T')?.[0]}T${event.extendedProps?.endDate?.split('T')?.[1]}`
        }
    }
    ...
    const formatDateToISOString = date => {
        ...
        return new Date(dateValue.getTime() - dateValue.getTimezoneOffset() * 60000).toISOString().split('.')[0]
    }
    ...
    const persistEventsInLocalStorage = (event = {}, index = -1, editing = false) => {
        const values = events
        ...
        if (index !== -1) {
            ...
            if (Object.keys(event || [])?.length) localStorage.setItem('events', JSON.stringify([...values, event]))
        ...
        if (values?.length > 0 && Object.keys(event || [])?.length) localStorage.setItem('events', JSON.stringify([...values, event]))
        else if (values?.length > 0 && !Object.keys(event || [])?.length) localStorage.setItem('events', JSON.stringify([...values]))
    ...
    const handleEventRepetition = value => {
        ...
        setEventInfo({...eventInfo, repeat: value, daysOfWeek: options[value], createdAt: new Date()})
    }
    ...
    const formatDate = date => {
        ...
        return new Date(date).toLocaleDateString(language, { day: 'numeric', month: 'long', year: 'numeric' })
    }
    
    const formatTime = date => {
        if (!date) return ''

        let time = new Date(date).toLocaleTimeString(language, { hour: 'numeric', minute: 'numeric', hour12: true })
        ...
        if (startDate === endDate && !eventInfo.allDay) {
            if (startPeriod === endPeriod) {
                return (
                    <div className='content__date content__info'>
                        <MdOutlineAccessTime />
                        {startDate}, {startTime.slice(0, -2)} {textTransform('lowercase', translate('CALENDAR.UNTIL'))} {endTime}
                    </div>
                )
            } else {
                return (
                    <div className='content__date content__info'>
                        <MdOutlineAccessTime />
                        {startDate}, {startTime} {textTransform('lowercase', translate('CALENDAR.UNTIL'))} {endTime}
                    </div>
                )
            }
        } else {
    ...
    const renderEventContent = (eventInfo) => {
        ...
        return <div className='fc-event-content'>
    ...
    const convertToIsoDate = date => {
        ...
        const newDate = new Date(parts[2], parts[1] - 1, parts[0], parts[3] || 0, parts[4] || 0)
    ...
    window.getFilterFc = (element, value) => {
        ...
        const eventsFilteredByType = events?.filter(event => filteredTypes.includes(event.type))
    ...
    return <AppPage 
        ...
        className={`calendar-page ${theme}`}
        breadcrumbs={[
            { label: "Magnun", url: "" },
            { label: translate('CALENDAR.TITLE') },
        ]}
        actions={<Dropdown 
            trigger={<Button title={translate('UI.CREATE')} appearance='primary' useIcon={{
                direction: 'left',
                icon: <MdAdd />
            }}/>}
        ...
        >
            <DropdownContent className='create-event-container'>
                {typesEvent?.map((type, index) => (
                    <DropdownItem key={index} onClick={() => createEventFc(type.type, type.value)}>
    ...
    >
        ...
        <div ref={eventDropdowRef} style={dropdownStyle} className='event-dropdown'>
            <div className='d-flex align-items-center justify-content-end gap-1 u-pointer mb-3 pr-3'>
                {eventInfo.url && <ActionButton onClick={() => openMeetingLink(eventInfo.url)} title={translate('CALENDAR.MEETING_LINK')}>
                    <MdOutlineDuo className='event-link' />
                </ActionButton>}

                <ActionButton onClick={() => handleEventEdit()} title={translate('UI.EDIT')}>
                    <MdOutlineModeEditOutline />
                </ActionButton>

                <ActionButton onClick={() => handleEventDeletion(eventInfo)} title={translate('UI.DELETE')}>
                    <MdOutlineDeleteOutline />
                </ActionButton>

                <ActionButton onClick={() => handleCloseDropdown()} title={translate('UI.CLOSE')}>
                    <MdOutlineClose />
                </ActionButton>
            </div>
            ...
            <div className='event-dropdown__content'>
                ...
                {eventInfo?.repeat && eventInfo?.repeat !== 'n' && <div className='content__repeat content__info'>
                    {eventInfo?.repeat === 'daily' && <span>{translate('CALENDAR.EVERY_DAY')}</span>}
                    {eventInfo?.repeat === 'weekly' && <span>{translate('DAYSOFWEEK.WEEKLY')}: {translate('FRAGMENTS.EVERY')} {daysOfWeek[new Date(eventInfo?.start?.split('T')).getDay()]}</span>}
                </div>}

                {eventInfo?.guests?.length > 0 && <div className='content__guests content__info'>
                    <MdOutlinePeopleOutline style={{ fontSize: '22px', marginTop: '2px' }} />
                
                    <Accordion 
                        items={[{title: `${eventInfo?.guests?.length} ${translate('UI.GUESTS')}`}]} 
                        borderless={true}
                    ...
                    <AccordionItem className='p-5'>
                            {
                                users.filter(user => eventInfo?.guests?.includes(user.value))?.map((user, index) => (
                                    <div key={index} className='d-flex align-items-center gap-1'>
                                        <Avatar image={user?.image} name={user.label} size='small' />
                ...
                {eventInfo?.description && <div className='content__description content__info'>
                ...
                <div className='content__author content__info'>
                    <Avatar image={eventInfo?.createdBy?.image} name={eventInfo?.createdBy.name} size='small'/>
        ...
        >
            <ModalHeader closeButton={true} onClose={handleCloseModal}>{translate('UI.CREATE')} {eventType?.toLowerCase()}</ModalHeader>
            <ModalBody>
                <div className='d-fex flex-column'>
                    <Input 
                        ...
                        onChange={(value) => setEventInfo({...eventInfo, title: value})} 
                    />

                    <div className='d-flex align-items-center gap-1'>
                        {eventInfo?.allDay ? <DatePicker 
                            label={translate('FRAGMENTS.IN')}
                            value={eventInfo?.formattedStartDate}
                            onChange={(value) => setEventInfo({...eventInfo, start: convertToIsoDate(value), startDate: convertToIsoDate(value)})}
                        /> : <DateTimerPicker 
                            label={translate('FRAGMENTS.IN')}
                        ...
                        {eventInfo.allDay ? <DatePicker 
                            ...
                            value={eventInfo?.formattedStartDate}
                            onChange={(value) => setEventInfo({...eventInfo, end: convertToIsoDate(value), endDate: convertToIsoDate(value)})}
                        /> : <DateTimerPicker 
                            label={translate('FRAGMENTS.UNTIL')}
                            placement='right'
                            value={eventInfo?.formattedEndDate}
                    ...
                    <div className='d-flex align-items-center gap-1 mt-3'>
                        <Select 
                            ...
                            onSelected={(selected) => handleEventRepetition(selected[0].value)}
                        />
                        ...
                        <div className="w-20 mt-3">
                            <Checkbox
                                ...
                                value='allDay'
                                isChecked={true}
                                onChange={({checked}) => setEventInfo({...eventInfo, allDay: checked})}
                            />
                        </div>
                    </div>
                    ...
                    <Select 
                        ...
                        options={users}
                        onSelected={(selected) => setEventInfo({...eventInfo, guests: selected?.map(item => item.value)})}
                    />
                    ...
                    <div className='mb-4'>
                        ...
                        <RichTextEditor 
                            content={''} 
                            useBorder
                            onChange={({content}) => setEventInfo({...eventInfo, description: content?.html})}
                        />
                    </div>
                    ...
                    <Input 
                        ...
                        onChange={(value) => setEventInfo({...eventInfo, url: value})} 
                    />
                </div>
            </ModalBody>
            <ModalFooter>
                <ButtonGroup placement='end'>
                    <Button title={translate('UI.CANCEL')} appearance='soft-link' onClick={() => {setOpenModal(false); clearEventInfo()}}/>
                    <Button title={translate('UI.CREATE')} appearance="primary" onClick={() => handleEventInclusion(Number(eventInfo.id))}/>
                </ButtonGroup>
            </ModalFooter>
        </Modal>

        <Modal
            isOpen={viewEventMobile}
            width='small'
        >
            <ModalBody>
                <div className='event-dropdown' style={{
                    position: 'relative',
                    display: 'block',
                    boxShadow: 'unset',
                    border: 'unset',
        ...
        <Modal
            isOpen={deleteRecurringEvent} 
            width='small'
        >
            <ModalHeader>{translate('CALENDAR.DELETE_RECURRING_EVENT')}</ModalHeader>

            <ModalBody>
                <Radio
                    options={[
                        {value: '1', label: translate('CALENDAR.THIS_EVENT')},
                        {value: '2', label: translate('CALENDAR.THIS_AND_THE_FOLLOWING_EVENTS')},
                ...
                <ButtonGroup placement='end'>
                    <Button title={translate('UI.CANCEL')} appearance='soft-link' onClick={() => setDeleteRecurringEvent(false)}/>
                    <Button title={translate('UI.DELETE')} appearance="primary" size='small' onClick={() => deleteEvent(eventInfo)}/>
                </ButtonGroup>
            </ModalBody>
        </Modal>
    </AppPage>
