...
const Default = () => {
    ...
    return (
        <div className={`${styles.default__container} ${styles[theme]}`} ref={containerRef} data-theme={theme} data-sidebar-theme={sidebarTheme} data-top-menu-theme={topMenuTheme}>
            <div className={`${styles.default__sidebar} ${!openMenu ? `${styles.mobile__sidebar}` : ''} ${styles[theme]}`} ref={paretSidebarRef}>
                <Sidebar
                    createRef={sidebarRef}
                    toggledMenu={openMenu}
                    menuMobile={openMenuMobile}
                    className={openMenuMobile ? 'sidebar__sm__md' : ''}
                    onClose={({ hasChildrens }) => setOpenMenuMobile(hasChildrens ? true : false)}
                />
            </div>
            <div className={`${styles.default__main} ${!openMenu ? `${styles.mobile__main}` : ''} ${theme === 'dark' ? `${styles.dark}` : ''}`} id='default-main'>
                <div>
                    <AppHeader
                        toggleMenu={() => onChildChanged()}
                        toggleMenuMobile={() => { setOpenMenuMobile(!openMenuMobile) }}
                    />
                    <AppContent />
                </div>
                {!hideFooter && (
