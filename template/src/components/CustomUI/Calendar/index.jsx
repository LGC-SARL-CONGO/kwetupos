...
export default function Calendar ({selected, minDate, maxDate, onChange, className}) {
    ...
    useEffect(() => {
        setState(prevState => ({
            ...prevState,
            selected: selected ? moment(selected, 'DD/MM/YYYY') : moment().startOf('day')
        }));
    }, [selected]);
    ...
    const previous = () => {
        ...
        if (minDate  !== undefined || previousMonth.isSameOrAfter(minDate, 'month')) {
    ...
    return <div className={`calendar-container ${global.config.theme} ${className}`}>
            <div className='calendar-container__header'>
                <div className='header-controls'>
                    <div className={`header-controls__buttons ${!state.constrols.previous ? 'header-controls__buttons--disabled' : ''}`}>
                        <MdNavigateBefore onClick={() => previous()}/>
                    </div>

                    <div>
                        <span className={`size-14 weight-500`} style={{textTransform: 'capitalize'}}>{state.month.format('MMMM YYYY')}</span>
                    </div>

                    <div className={`header-controls__buttons ${!state.constrols.next ? 'header-controls__buttons--disabled' : ''}`}>
...
const Days = () => {
    return <div className='weekly-days'>
        {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((day, index) => <span key={index} className='day'>{day}</span>)}
    </div>
...
const Week = props => {
    ...
    return <div className='week' key={days[0]}>{days}</div>
}
...
const Day = (props) => {
    ...
    return (
      <div
        key={uuid()}
        className={dayClass}
        onClick={(event) => (isBeforeMinDate || isAfterMaxDate ? null : select(day, event))}
      >
        {number}
      </div>
    )
