...
export default function Maps({title, map, data, filter, showLabels, className, content}) {
    const [selectedMap, setSelectedMap] = useState({label: 'Brasil', value: 'brazil'})
    ...
    const onRegionTipShow = (e, el, code) => {
        ...
        el.html(`${el[0]?.innerText}: ${formatValue(value, 'number')} ${content || ''}`)
    }

    const handleClick = option => {
        const values = option?.value !== 'word' ? data?.seriesByRegion[option.value] : data?.series
    ...
    const getSeries = () => {
        const values = series
        if (values?.regions?.length) values.regions[0].scale = [theme === 'dark' ? '#3F486B' : '#BEC4D9']
    ...
    return <div className={`${styles['map-container']} ${className}`} data-theme={theme}>
        <div className={styles['map-container__header']}>
            <span className={styles['title']}>{title}</span>

            {filter && <div>
                <Dropdown 
                    trigger={<div className={styles['filter__dropdown']}>{selectedMap?.label}</div>}
                    placement='top-end'
                >
                    <DropdownContent>
