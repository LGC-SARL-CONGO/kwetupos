...
export default function GrowthRate({ currentValue, previousValue }) {
    ...
    return <div className='growth-rate-container'>
        <div className='growth-rate-container__rate'>
            <span className={`${percentage > 0 ? 'growth-rate-container__rate--positive' : 'growth-rate-container__rate--negative'} d-flex align-items-center`}>
                <span className='d-flex'>{percentage < 0 ? <MdOutlineTrendingDown/> : <MdOutlineTrendingUp />}</span>
