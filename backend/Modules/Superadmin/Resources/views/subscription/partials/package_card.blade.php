<div class="col-md-4 tw-mb-5 {{ $package->interval }} tw-relative">
    <div
        class="tw-flex tw-flex-col tw-gap-6 tw-p-6 tw-shadow card-blur-bg tw-rounded-2xl tw-shadow-lg tw-transition-all tw-duration-700 hover:tw-scale-110 tw-cursor-pointer" > 
        ...
        <div class="tw-flex tw-flex-col tw-text-center">
            ...
            <h3
                class="text-black tw-from-indigo-500 tw-inline-block tw-text-transparent tw-bg-clip-text tw-font-semibold tw-text-2xl md:tw-text-3xl">
                @php
        ...
        @else
            @if (isset($action_type) && $action_type == 'register')
                <a href="{{ route('business.getRegister') }}?package={{ $package->id }}"
                    class="tw-h-12 tw-rounded-xl tw-text-sm md:tw-text-base tw-text-white tw-font-semibold tw-w-full tw-max-w-full tw-mt-2 tw-flex tw-items-center tw-justify-center red-gradient focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-red-500 focus:tw-ring-offset-2">
                    @if ($package->price != 0)
